...
- **Coleta de Dados**: 
  - O `InstagramWorker` utiliza o motor Playwright (`scraper_headless.py`) para coleta via Modal.
  - O projeto `sentinela_novo` (Scrapy) está ativo para coleta via API GraphQL.
  - **Política de Sessão**: É terminantemente proibida a rotação diária de `sessionid`. Utilize uma sessão estável até o esgotamento (Erro 429). Em caso de 429, aplique *backoff* (repouso de conta) por 12-24h. Nunca forçar a substituição de token como estratégia de bypass.
