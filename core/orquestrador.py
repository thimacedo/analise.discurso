...
class Orchestrator:
    ...
    async def run_scraper(self, limit=200, cooldown=300):
        ...
        res = db.client.table('fila_coleta')\
            .select('candidato_id')\
    ...
    async def run_repericia_cycle(self):
        db = get_supabase_client() # Obtém o cliente Supabase
        targets = await db.fetch_targets_needing_repericia()
        if not targets: return
        print(f"🕵️‍♂️ Re-perícia: {', '.join(targets)}")
        for username in targets:
            await db.reset_target_comments(username)
            await db.mark_repericia_complete(username)
    ...
    async def persist_intelligence(self, df: pd.DataFrame, clusters: List[Dict]):
        ...
        if 'cluster' in df.columns:
            ...
            if updates: await db.batch_update_comments(updates)
        ...
        await db.upsert_daily_metrics({"p_data": hoje, "p_total_coletado": int(total), "p_total_hate": int(hate), "p_total_neutro": int(total - hate), "p_resiliencia": float(res), "p_pasa_breakdown": pasa})

        for c in clusters:
            if c['event_count'] < 3: continue
            severity = min(100, int((c['event_count'] / len(df)) * 1000)) if len(df) > 0 else 0
            await db.persist_coordinated_network({"nome": f"Rede {c['cluster_id']} - {datetime.now().strftime('%d/%m')}", "status": "ATIVA" if severity > 50 else "MONITORANDO", "eventos_count": c['event_count'], "severidade": severity, "palavras_chave": c['keywords']})
