"""
PASA v49 - AI Service: Motor de Inteligência Resiliente
...
SYSTEM_PROMPT = """Você é um analista forense digital do sistema Sentinela Democrática. Analise o comentário político abaixo e classifique seguindo o protocolo PASA. Responda APENAS com JSON válido contendo:
{
    "is_hate": boolean,
...
class AIService:
    ...
    async def classify_text(self, text: str) -> Dict[str, Any]:
        ...
        for provider in self.providers:
            ...
            try:
                response = await provider["client"].chat.completions.create(
                    ...
                    temperature=0.1,  # Baixa criatividade, alta precisão forense
                    timeout=15.0
                )
        ...
        # Se chegou aqui, TODAS as IAs falharam ou estão com circuito aberto
        raise RuntimeError("Todas as APIs de IA estão indisponíveis ou com circuito aberto.")

    def _parse_json_response(self, content: str) -> Dict[str, Any]:
        """Extrai e valida o JSON retornado pela IA."""
        try:
            data = json.loads(content)
            # Normalização e fallbacks seguros
            return {
