...
app = FastAPI(title="ForenseNet API v5.4.1 - Visual OSINT Fixed", version="5.4.1")
...
# --- FUNÇÕES DE SEGURANÇA (DEFINIR ANTES DAS ROTAS) ---
...
<body class="antialiased">
        ...
        <main class="flex-1 overflow-y-auto">
            ...
            <div class="p-16 max-w-7xl mx-auto space-y-16">
                ...
                <div id="view-intelligence" class="view-content hidden max-w-5xl mx-auto space-y-16">
                     <div class="text-center space-y-4"><h2 class="text-8xl font-800 tracking-tighter uppercase italic leading-none">IA <span class="text-blue-900">Lab</span></h2><p class="text-slate-400 font-bold uppercase tracking-[0.5em] text-xs italic">Decomposição Semântica v5.4</p></div>
    ...
    <script>
        ...
        async function refreshAll() { document.getElementById('last-update').innerText = "SYNC: " + new Date().toLocaleTimeString('pt-BR'); await loadStats(); await loadComments(); }
        function annotateText(text) { if(!text) return ""; let annotated = text; HATE_KEYWORDS.forEach(word => { const regex = new RegExp(`\\\\b(${word})\\\\b`, 'gi'); annotated = annotated.replace(regex, `<span class="highlight-hate" title="Dicionário Pericial: Gatilho Detectado">$1</span>`); }); return annotated; }
        async function loadStats() { try { const headers = { "apikey": SB_KEY, "Authorization": `Bearer ${SB_KEY}`, "Prefer": "count=exact" }; const resTotal = await fetch(`${SB_URL}/comentarios?select=id`, { headers }); const total = parseInt(resTotal.headers.get("content-range")?.split("/")[1]) || 0; const resHate = await fetch(`${SB_URL}/comentarios?is_hate=eq.true&select=id`, { headers }); const hate = parseInt(resHate.headers.get("content-range")?.split("/")[1]) || 0; document.getElementById('kpi-total').innerText = total.toLocaleString(); document.getElementById('kpi-hate').innerText = hate.toLocaleString(); document.getElementById('kpi-percent').innerText = total > 0 ? `${((hate/total)*100).toFixed(1)}%` : "0.0%"; } catch(e) { console.error(e); } }
        async function loadComments() { try { const res = await fetch(`${SB_URL}/comentarios?select=*&order=data_coleta.desc&limit=100`, { headers: { "apikey": SB_KEY, "Authorization": `Bearer ${SB_KEY}` } }); const data = await res.json(); const grouped = data.reduce((acc, c) => { const key = c.post_url || 'unknown'; if (!acc[key]) acc[key] = { candidate: c.candidato_id, date: c.data_coleta, image: c.post_image, caption: c.post_caption, comments: [] }; acc[key].comments.push(c); return acc; }, {}); const container = document.getElementById('evidence-container'); container.innerHTML = Object.entries(grouped).map(([postKey, group]) => { const postUrl = postKey !== 'unknown' ? (postKey.startsWith('http') ? postKey : `https://www.instagram.com/p/${postKey}/`) : '#'; const hateCount = group.comments.filter(c => c.is_hate).length; const hasHighRisk = hateCount > group.comments.length * 0.3; return `
        ...
        function switchView(viewId) { document.querySelectorAll('.view-content').forEach(v => v.classList.add('hidden')); document.getElementById(`view-${viewId}`).classList.remove('hidden'); document.querySelectorAll('.nav-button').forEach(n => n.classList.remove('active')); document.getElementById(`nav-${viewId}`).classList.add('active'); }
        window.onload = refreshAll; setInterval(refreshAll, 60000);
    </script>
</body>
</html>
"""
    ...
    try:
        with db.get_session() as session:
            ...
            return [{
                ...
                "post_url": c.post_id,
                "post_image": c.id_externo.split('_')[0] if not c.likes else None, 
        ...
        for item in payload:
            try:
                ...
                if not session.query(Comentario).filter(Comentario.id_externo == id_ext).first():
                    ...
                    cl = Classificacao(comentario_id=com.id, is_hate=is_h, categoria_odio=item.get('categoria'), score=item.get('score'), modelo_versao='qwen-v5.4-fixed')
...
@app.get("/api")
def read_api_root(): return {"status": "ForenseNet API v5.4.1 Online"}
