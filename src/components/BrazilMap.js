// SENTINELA MAP v17.0 - ULTIMATE STANDARD GEOGRAPHY
// Modelo de alta fidelidade cartográfica com proporções reais (IBGE Style).

export const brazilPaths = {
    "AC": "M60.6,281.3l-5.6-7.8l23.7-17.9l21.8,9.7l13.1-18.4l22.9,6.4l6,26.5l-3.8,7.2l-21.7,18.6l-23.2-0.5L60.6,281.3z",
...
export function renderBrazilMap(containerId, stats) {
    ...
    svg.innerHTML = `
        ...
        ${Object.entries(brazilPaths).map(([uf, d]) => {
            ...
            const intensity = Math.min(info.count * 15, 100);
            const fill = info.count > 0 ? "rgba(59, 130, 246, " + (0.4 + (intensity/150)) + ")" : 'rgba(30, 41, 59, 0.5)';
            const stroke = info.hate > 0 ? '#ef4444' : '#475569';
            
            return '<path id="state-' + uf + '" d="' + d + '" class="state' + (info.hate > 0 ? ' danger-node' : '') + '" ' +
                  'fill="' + fill + '" ' +
                  'stroke="' + stroke + '" ' +
                  'stroke-width="' + (info.hate > 0 ? '2' : '0.5') + '" ' +
                  'style="' + (info.hate > 0 ? 'filter: url(#glow-intel);' : '') + '" ' +
