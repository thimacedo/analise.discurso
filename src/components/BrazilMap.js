...
export function renderBrazilMap(containerId, stats) {
    ...
    // ViewBox calibrado para a grade quadrada
    ...
    svg.innerHTML = `
        ...
        ${Object.entries(gridLayout).map(([uf, pos]) => {
            ...
            const fill = info.count > 0 ? (hasHate ? 'rgba(239, 68, 68, 0.5)' : 'rgba(59, 130, 246, 0.4)') : 'rgba(30, 41, 59, 0.6)';
            ...
            <g class="state-group cursor-pointer transition-all duration-300 hover:opacity-80" onclick="window.focusState('${uf}')">
                ...
                <text x="${x + boxSize/2}" y="${y + boxSize/2}" 
                      ...
                      fill="${info.count > 0 ? '#fff' : '#475569'}" 
                      class="text-xs font-black pointer-events-none tracking-tighter">
                    ${uf}
                </text>
