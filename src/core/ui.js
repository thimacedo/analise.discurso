/**
 * SENTINELA UI ENGINE v15.7.1
 * Monitor de Impacto & Hostilidade v3.0
 */
...
/**
 * MONITOR DE IMPACTO & HOSTILIDADE v3.0
 * Portado do componente React: MonitorImpactoHostilidade.jsx
 */
async function renderMonitorImpacto() {
    ...
    try {
        ...
        if(!alvos || alvos.length === 0) {
            container.innerHTML = '<p class="text-[10px] text-slate-500 animate-pulse text-center py-10 uppercase font-black tracking-widest">Sincronizando Motor PASA v3.0...</p>';
            return;
        }
            ...
            return `
                <div onclick="window.openDetail('${alvo.username}')" 
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <div class="flex items-center gap-4 text-right border-l border-white/5 pl-4">
                            <div class="flex flex-col items-end">
                                <span class="text-[9px] font-black text-slate-500 uppercase tracking-wider mb-0.5">Prote&ccedil;&atilde;o</span>
                                <span class="text-xs font-black text-emerald-400 font-mono">${blindagem.toFixed(1)}%</span>
                            </div>
                            <div class="flex flex-col items-end">
                                <span class="text-[9px] font-black text-slate-500 uppercase tracking-wider mb-0.5">Risco</span>
                    ...
                    <div class="w-full h-2 bg-slate-900 rounded-full overflow-hidden flex shadow-inner">
                        <div 
                            class="h-full bg-gradient-to-r from-blue-600 to-blue-400 transition-all duration-1000 ease-out" 
                            style="width: ${blindagem}%"
                        ></div>
                        <div 
                            class="h-full bg-gradient-to-r ${isCritical ? 'from-rose-500 to-red-600' : 'from-slate-700 to-slate-600'} transition-all duration-1000 ease-out" 
                            style="width: ${hostilidade}%"
                        ></div>
                    </div>
                </div>`;
        }).join('');
        ...
        console.error("Monitor v3 Error:", e);
    }
}

// Restante das funções auxiliares mantidas (Dossie, Map, etc) com o novo estilo v3.0
