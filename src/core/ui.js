/**
 * SENTINELA UI ENGINE v15.6.6
 * Ultra Performance - View-Aware Rendering
 */
...
export function renderAll() {
    try {
        // KPIs são globais e leves, podem ser renderizados sempre
        renderKPIs();

        // Renderização Condicional por View (Otimização de INP)
        ...
        // Processamento de ícones apenas se necessário
        if (window.lucide) {
            lucide.createIcons();
        }
    } catch (e) {
...
function renderDossieGrid() {
    ...
    const renderBatch = (list, title) => {
        ...
        return `<div class="col-span-full mt-8 mb-4 flex items-center gap-4"><h3 class="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">${title}</h3><div class="h-px bg-white/5 w-full"></div></div>${list.map(t => {
            const resM = (t.comentarios_totais_count || 0) > 0 ? (100 - (t.comentarios_odio_count / t.comentarios_totais_count * 100)).toFixed(1) : "100.0";
            return `<div class="glass-card p-6 border-white/5 hover:border-blue-500/30 transition-all flex flex-col justify-between h-full bg-slate-900/20"><div class="flex items-center gap-3 mb-6">${imgHtml(cleanName(t), "w-12 h-12 rounded-xl")}<div class="min-w-0"><h4 class="text-[10px] font-black text-white truncate">@${cleanName(t)}</h4><span class="text-[7px] text-blue-400 uppercase font-black tracking-widest">${t.estado || 'BR'}</span></div></div><div class="space-y-3"><div class="flex justify-between items-end"><span class="text-[8px] text-slate-500 font-black uppercase tracking-tighter">Blindagem</span><span class="text-[10px] font-black ${resM < 70 ? 'text-red-500' : 'text-emerald-500'} font-mono">${resM}%</span></div><div class="w-full h-1 bg-white/5 rounded-full overflow-hidden"><div class="h-full ${resM < 70 ? 'bg-red-600 shadow-[0_0_8px_#ef4444]' : 'bg-blue-600'}" style="width: ${resM}%"></div></div></div><button onclick="window.openDetail('${t.username}')" class="mt-6 w-full py-2.5 bg-white/5 hover:bg-blue-600/20 text-blue-400 text-[8px] font-black border border-white/5 rounded-lg transition-all uppercase tracking-widest">Dossiê Detalhado</button></div>`;
        }).join('')}`;
    };
