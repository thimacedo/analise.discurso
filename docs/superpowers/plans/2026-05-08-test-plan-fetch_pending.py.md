# Plano de Implementação de Testes para fetch_pending.py

> **Para agentes de execução:** HABILIDADE OBRIGATÓRIA: Use `superpowers:subagent-driven-development` (recomendado) ou `superpowers:executing-plans` para implementar este plano tarefa a tarefa. As etapas usam a sintaxe de caixa de seleção (`- [ ]`) para rastreamento.

**Objetivo:** Garantir que o script `fetch_pending.py` funcione corretamente, lidando com diversos cenários de dados e erros, e que o mock do `SupabaseService` e `settings` seja configurado adequadamente para testes.

**Arquitetura:** Testes unitários para `fetch_pending.py` usando `pytest`. Mocking de dependências (`core.supabase_service` e `core.config`) para isolar o script a ser testado.

**Tech Stack:** Python, pytest, unittest.mock (ou similar para mocking).

---

### Tarefa 1: Configurar Mock do SupabaseService e Settings

**Arquivos:**
- Criar: `tests/mocks/mock_supabase_service.py`
- Criar: `tests/mocks/mock_config.py`
- Modificar: `tests/test_fetch_pending.py`

- [ ] **Etapa 1: Criar `tests/mocks/mock_supabase_service.py`**
    Este arquivo conterá uma classe mock para `SupabaseService` com métodos simulados para `fetch_unprocessed_comments` e `update_comment_classification`.
...
- [ ] **Etapa 2: Criar `tests/mocks/mock_config.py`**
    Este arquivo conterá um objeto mock `settings` com as variáveis necessárias (`SUPABASE_URL`, `SUPABASE_KEY`).
...
- [ ] **Etapa 3: Criar `tests/test_fetch_pending.py` e importar mocks**
    Este arquivo conterá os testes unitários. Inicialmente, vamos configurar o mock.
    ...
    async def test_fetch_and_process_pending_success(self, patch_dependencies):
        ...
        # Verificar logs impressos (opcional, mais para debug manual)
        # print("Output log do teste test_fetch_and_process_pending_success")
    ...
    async def test_fetch_and_process_pending_empty_text_fields(self, patch_dependencies):
        ...
        # Verifica se o contador de ignorados está correto
        # A lógica atual não imprime um contador explícito para "ignorados_vazios" fora do resumo.
        # Precisaríamos adicionar prints no script original para testar isso diretamente.
        # Para este teste, apenas verificar se o número de atualizações está correto é suficiente.
        # O script original imprime um resumo, que podemos verificar se a contagem estiver correta.

    ```

- [ ] **Etapa 4: Executar testes para verificar falhas iniciais**
    Verificar se os testes falham como esperado (e.g., `NameError` para `fetch_pending` não ter mocks configurados).

    Run: `pytest tests/test_fetch_pending.py`
    Expected: FAIL (e.g., `NameError: name 'supabase_client_instance' is not defined` or `AttributeError: 'MagicMock' object has no attribute 'fetch_unprocessed_comments'`)

- [ ] **Etapa 5: Implementar o código de patch no `tests/test_fetch_pending.py`**
    Ajustar o arquivo de teste para que os mocks sejam aplicados corretamente antes da importação do módulo a ser testado.

    *Nota:* O código na Etapa 3 já inclui a configuração de patch usando `pytest` e `module_mocker`.

- [ ] **Etapa 6: Executar testes para verificar se passam**
    Rodar os testes novamente após garantir que os mocks estão corretamente configurados e injetados.

    Run: `pytest tests/test_fetch_pending.py`
    Expected: PASS para todos os testes.

- [ ] **Etapa 7: Adicionar `tests/mocks/__init__.py`**
    Criar um arquivo `__init__.py` vazio dentro de `tests/mocks/` para torná-lo um pacote Python.

- [ ] **Etapa 8: Commit**

```bash
git add tests/test_fetch_pending.py tests/mocks/mock_supabase_service.py tests/mocks/mock_config.py tests/mocks/__init__.py
git commit -m "test: Add initial unit tests and mocks for fetch_pending.py"
```

### Tarefa 2: Testar o fluxo de sucesso

**Arquivos:**
- Modificar: `tests/test_fetch_pending.py`

- [ ] **Etapa 1: Configurar o retorno do `fetch_unprocessed_comments` para um cenário de sucesso**
    Definir dados de teste que simulam comentários encontrados, incluindo um caso para o fallback de `texto_bruto`.

- [ ] **Etapa 2: Executar o teste `test_fetch_and_process_pending_success`**

    Run: `pytest tests/test_fetch_pending.py::test_fetch_and_process_pending_success -v`
    Expected: PASS

- [ ] **Etapa 3: Verificar as chamadas para `update_comment_classification`**
    Garantir que o mock registrou as chamadas corretas com os dados esperados (usando `texto_limpo` e `texto_bruto` conforme a lógica do script).

- [ ] **Etapa 4: Commit**

```bash
git add tests/test_fetch_pending.py
git commit -m "test: Verify successful fetching and processing in fetch_pending.py"
```

### Tarefa 3: Testar o cenário sem comentários pendentes

**Arquivos:**
- Modificar: `tests/test_fetch_pending.py`

- [ ] **Etapa 1: Configurar o retorno do `fetch_unprocessed_comments` para uma lista vazia**
    Simular o caso em que não há comentários pendentes.

- [ ] **Etapa 2: Executar o teste `test_fetch_and_process_pending_no_comments`**

    Run: `pytest tests/test_fetch_pending.py::test_fetch_and_process_pending_no_comments -v`
    Expected: PASS

- [ ] **Etapa 3: Verificar que `update_comment_classification` não foi chamado**
    Confirmar que nenhuma atualização foi tentada quando não há comentários para processar.

- [ ] **Etapa 4: Commit**

```bash
git add tests/test_fetch_pending.py
git commit -m "test: Verify fetch_pending.py handles no pending comments scenario"
```

### Tarefa 4: Testar o tratamento de erros na busca

**Arquivos:**
- Modificar: `tests/test_fetch_pending.py`

- [ ] **Etapa 1: Configurar o mock `supabase_client_instance` para lançar uma exceção em `fetch_unprocessed_comments`**
    Simular um erro de rede ou conexão.

- [ ] **Etapa 2: Executar o teste `test_fetch_and_process_pending_fetch_error`**

    Run: `pytest tests/test_fetch_pending.py::test_fetch_and_process_pending_fetch_error -v`
    Expected: PASS

- [ ] **Etapa 3: Verificar se a mensagem de erro correta é impressa**
    Confirmar que o script imprime a mensagem de erro apropriada e não lança exceções não tratadas.

- [ ] **Etapa 4: Commit**

```bash
git add tests/test_fetch_pending.py
git commit -m "test: Verify fetch_pending.py handles fetch errors gracefully"
```

### Tarefa 5: Testar o tratamento de erros na atualização

**Arquivos:**
- Modificar: `tests/test_fetch_pending.py`

- [ ] **Etapa 1: Configurar o mock `supabase_client_instance` para lançar uma exceção em `update_comment_classification`**
    Simular um erro ao tentar atualizar o registro no banco de dados.

- [ ] **Etapa 2: Executar o teste `test_fetch_and_process_pending_update_error`**

    Run: `pytest tests/test_fetch_pending.py::test_fetch_and_process_pending_update_error -v`
    Expected: PASS

- [ ] **Etapa 3: Verificar se mensagens de erro específicas para atualização são impressas**
    Confirmar que o script registra falhas de atualização por comentário e relata o total de falhas.

- [ ] **Etapa 4: Commit**

```bash
git add tests/test_fetch_pending.py
git commit -m "test: Verify fetch_pending.py handles update errors gracefully"
```

### Tarefa 6: Testar o tratamento de IDs ausentes

**Arquivos:**
- Modificar: `tests/test_fetch_pending.py`

- [ ] **Etapa 1: Configurar o retorno do `fetch_unprocessed_comments` para incluir um registro sem `id`**
    Simular um dado malformado vindo do banco.

- [ ] **Etapa 2: Executar o teste `test_fetch_and_process_pending_missing_id`**

    Run: `pytest tests/test_fetch_pending.py::test_fetch_and_process_pending_missing_id -v`
    Expected: PASS

- [ ] **Etapa 3: Verificar se o registro sem ID é ignorado e se uma mensagem de aviso é impressa**
    Confirmar que o script não falha e registra o problema.

- [ ] **Etapa 4: Verificar que `update_comment_classification` só é chamado para registros com ID**

- [ ] **Etapa 5: Commit**

```bash
git add tests/test_fetch_pending.py
git commit -m "test: Verify fetch_pending.py handles records with missing IDs"
```

### Tarefa 7: Testar o tratamento de campos de texto vazios/nulos

**Arquivos:**
- Modificar: `tests/test_fetch_pending.py`

- [ ] **Etapa 1: Configurar o retorno do `fetch_unprocessed_comments` para incluir um registro onde `texto_bruto` e `texto_limpo` são vazios ou nulos**
    Simular um registro onde não há texto para analisar.

- [ ] **Etapa 2: Executar o teste `test_fetch_and_process_pending_empty_text_fields`**

    Run: `pytest tests/test_fetch_pending.py::test_fetch_and_process_pending_empty_text_fields -v`
    Expected: PASS

- [ ] **Etapa 3: Verificar que `update_comment_classification` não é chamado para registros sem texto**
    Confirmar que o script ignora registros que não possuem texto em `texto_limpo` nem `texto_bruto`.

- [ ] **Etapa 4: Commit**

```bash
git add tests/test_fetch_pending.py
git commit -m "test: Verify fetch_pending.py handles records with empty/null text fields"
```

---
Plano completo e salvo em `docs/superpowers/plans/2026-05-08-test-plan-fetch_pending.py.md`.

Duas opções de execução:

1.  **Orientado por Subagente (recomendado)** - Despacho um novo subagente para cada tarefa, reviso entre tarefas, iteração rápida.
2.  **Execução Inline** - Executo tarefas nesta sessão usando `executing-plans`, execução em lote com checkpoints.
