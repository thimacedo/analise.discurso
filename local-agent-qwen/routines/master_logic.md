# 🧠 Lógica Master de Automação (Refinada)

## 🛡️ DIRETRIZES DE ESTADO
- **Modo Gradual**: Execute uma sub-etapa por vez e reporte antes de seguir.
- **Limpeza de Recursos**: Antes de tarefas pesadas, verifique processos que consomem muita CPU/RAM (ex: Chrome, Edge, VSCode não utilizados) e encerre-os se o ambiente for "dedicado ao agente".

## 🛠️ FERRAMENTAS DE OTIMIZAÇÃO (PowerShell)
- Listar processos pesados: `Get-Process | Sort-Object CPU -Descending | Select-Object -First 5`
- Matar processo por nome: `Stop-Process -Name "nome_do_processo" -Force`

## 🗺️ FLUXO FASE 1 (GRADUAL)
1. **Schema**: Crie apenas `database/models.py`. Validar sintaxe. Reportar.
2. **Repository**: Criar `database/repository.py` com funções CRUD básicas. Reportar.
3. **Migration**: Criar script para ler CSV antigo e salvar no Postgres. Reportar.
4. **Refactor**: Alterar o `coletor.py` para usar o repositório. Reportar.
